...
def run_test():
    file_path = 'semi_supervised_null_model_auc_samples.json'
    data = GLV.generate_models(m, 1, file_path, force=True, bound=0.025)
    data['models'].append({
        'r': data['models'][0]['r'],
        'A': np.diag([-1] * GLV.GLV.numOfPopulations),
        'cohort': GLV.GLV(r=data['models'][0]['r'], A=np.diag([-1] * GLV.GLV.numOfPopulations)).get_samples(m),
        'index': 1
    })
