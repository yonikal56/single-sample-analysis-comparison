...
healthy_cohort = np.array([(np.array(df[col].tolist())/sum(df[col].tolist()))[:-1] for col in group_1_columns])
other_cohort = np.array([(np.array(df[col].tolist())/sum(df[col].tolist()))[:-1] for col in group_2_columns])
